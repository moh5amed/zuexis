...
export const useCloudStorage = () => {
  ...
  useEffect(() => {
    if (user) {
      console.log('🔍 [useCloudStorage] User authenticated, loading providers...');
      const loadProviders = async () => {
        try {
          ...
          console.log('📥 [useCloudStorage] Providers loaded:', { allProviders, connected });
          setProviders(allProviders);
          setConnectedProviders(connected);
        } catch (err) {
          console.error('❌ [useCloudStorage] Failed to load providers:', err);
      ...
      console.log('🔍 [useCloudStorage] No user, clearing providers');
      setProviders([]);
  ...
  useEffect(() => {
    if (!user) {
      console.log('🔍 [useCloudStorage] No user, skipping connection status check');
      return;
    }

    const checkConnectionStatus = async () => {
      try {
        console.log('🔄 [useCloudStorage] Checking connection status...');
        // Check real-time status of all connected providers
        const realTimeConnected = await cloudStorageService.getConnectedProviders();
        console.log('📊 [useCloudStorage] Connection status updated:', realTimeConnected);
        setConnectedProviders(realTimeConnected);
      } catch (err) {
        console.error('❌ [useCloudStorage] Failed to check connection status:', err);
        // Don't set error here as it's just a background check
      }
    };
    ...
    return () => {
      console.log('🧹 [useCloudStorage] Cleaning up connection status interval');
  ...
  return {
    ...
    connectOneDrive: cloudStorageService.connectOneDrive.bind(cloudStorageService),
    connectDropbox: cloudStorageService.connectDropbox.bind(cloudStorageService),
