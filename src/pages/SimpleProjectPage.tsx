...
const SimpleProjectPage = () => {
  ...
  console.log('🔍 [SimpleProjectPage] Component rendered');
  console.log('🔍 [SimpleProjectPage] URL params:', useParams());
  console.log('🔍 [SimpleProjectPage] Project ID from params:', id);
  console.log('🔍 [SimpleProjectPage] Current window location:', window.location.href);
  ...
  useEffect(() => {
    console.log('🔍 [SimpleProjectPage] useEffect triggered with ID:', id);
    if (id) {
      console.log('🚀 [SimpleProjectPage] Loading project with ID:', id);
      loadProject();
    } else {
      console.log('❌ [SimpleProjectPage] No project ID provided');
    }
  }, [id]);

  const loadProject = async () => {
    try {
      console.log('🔍 [SimpleProjectPage] loadProject called with ID:', id);
      setLoading(true);
      if (!id) {
        console.log('❌ [SimpleProjectPage] No ID provided to loadProject');
        return;
      }

      console.log('🔍 [SimpleProjectPage] Querying Supabase for project ID:', id);
      ...
      console.log('📥 [SimpleProjectPage] Supabase response:', { projectData, projectError });
      
      if (projectError || !projectData) {
        console.error('❌ [SimpleProjectPage] Project not found:', projectError);
        showToast('Project not found', 'error');
        navigate('/projects');
        return;
      }

      console.log('✅ [SimpleProjectPage] Project loaded successfully:', projectData);
      setProject(projectData);
      
      // Load clips from Python backend
      try {
        const response = await fetch(`https://zuexisbacckend.onrender.com/api/frontend/get-latest-clips`);
        if (response.ok) {
