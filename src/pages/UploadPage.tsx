...
import { useFocusedVideoClipper } from '../hooks/useFocusedVideoClipper';
...
import { backgroundBackendTest } from '../services/backgroundBackendTest';
...
const UploadPage = () => {
  ...
  // 🎬 INTEGRATE FOCUSED VIDEO CLIPPER BACKEND FOR VIDEO PROCESSING
  const {
  ...
  // Loading screen state (only used during video processing)
  ...
  // 🛠️ DEVELOPER MODE - Bypass all subscription limits
  const [developerMode, setDeveloperMode] = useState(false);

  // 🔍 Backend health test on page load (silent, anonymous)
  useEffect(() => {
    // Run backend health test only when user opens the upload page
    const runInitialHealthTest = async () => {
      try {
        await backgroundBackendTest.forceTest();
      } catch (error) {
        // Silent failure - health test should not interrupt user experience
      }
    };
    
    runInitialHealthTest();
  }, []);
  ...
  // 🔐 Set current user for services when user changes (DISABLED to prevent interference with uploads)
  useEffect(() => {
    if (user?.id) {
      // DISABLED: Setting current user to prevent interference with uploads
      // supabaseProjectsService.setCurrentUser(user.id);
      // subscriptionService.setCurrentUser(user.id);
    }
  }, [user]);

  // 🔍 Helper function to check backend health before processing
  const checkBackendHealth = async (): Promise<boolean> => {
    try {
      const testResult = await backgroundBackendTest.forceTest();
      return testResult ? testResult.isConnected : false;
    } catch (error) {
      return false;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check subscription limits first (skip if developer mode is enabled)
    if (!developerMode && !await checkProjectAccess()) {
      showToast('You have reached your monthly video limit. Please upgrade your plan or wait until next month.', 'error');
      return;
    }

    if (developerMode) {
      console.log('🛠️ [Developer Mode] Bypassing subscription limits - unlimited processing enabled');
    }
    ...
    // 🔍 Run background backend test before processing (silent, anonymous)
    const isBackendHealthy = await checkBackendHealth();
    if (!isBackendHealthy) {
      showToast('Backend service is currently unavailable. Please try again later.', 'error');
      return;
    }

    setProcessing(true);
    
    // Show loading screen for video processing
    setShowLoadingScreen(true);
    setCurrentStageIndex(0);
    updateLoadingStage(0, 'processing', 0, 'Preparing video for upload...');
    ...
    try {
      ...
      if (uploadMethod === 'file' && file) {
        ...
        // Skip preflight check to focus on upload - terminate unnecessary operations
        console.log('🚀 [UploadPage] Starting direct video processing (skipping preflight checks)...');
        ...
        // Use focused backend for processing
        const processVideoFunction = processVideoFocused;
        
        // Add timeout wrapper for the entire processing operation
        const processingPromise = processVideoFunction({
          ...
          // Update progress for chunked uploads
          if (progress && typeof progress === 'object') {
            // Progress updates are now handled by the loading screen
          }
        });

        // Race between processing and timeout
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Video processing timeout after 35 minutes')), 35 * 60 * 1000)
        );
        ...
        if (processingResult) {
          ...
          const clips = (processingResult as any).clips;
          if (connectedProviders.length > 0 && processingResult && typeof processingResult === 'object' && processingResult !== null && 'clips' in processingResult && Array.isArray(processingResult.clips) && processingResult.clips.length > 0) {
            console.log(`☁️ [UploadPage] Step 2: Uploading generated clips to cloud storage...`);
            
            // Show full video transcription if available
            const transcription = (processingResult && typeof processingResult === 'object' && processingResult !== null && 'fullVideoTranscription' in processingResult ? processingResult.fullVideoTranscription : 
                                 processingResult && typeof processingResult === 'object' && processingResult !== null && 'transcription' in processingResult ? processingResult.transcription : null) as string | null;
            if (transcription) {
              console.log(`📝 [UploadPage] Full video transcription available: ${transcription.substring(0, 100)}...`);
            }

                const connectedProvider = connectedProviders[0];
            console.log(`☁️ [UploadPage] Uploading ${clips.length} clips to ${connectedProvider.name}...`);
            ...
            for (const clip of clips) {
                  try {
                    ...
                    if (clip.transcription || clip.segment_text || clip.caption) {
                      try {
                        console.log(`📝 [UploadPage] Uploading transcription for clip ${clip.clip_number}...`);
                        const transcriptionText = clip.transcription || clip.segment_text || clip.caption || '';
              ...
              if ((processingResult && typeof processingResult === 'object' && processingResult !== null && 'fullVideoTranscription' in processingResult && processingResult.fullVideoTranscription) || 
                  (processingResult && typeof processingResult === 'object' && processingResult !== null && 'transcription' in processingResult && processingResult.transcription)) {
                try {
                  console.log(`📝 [UploadPage] Uploading full video transcription...`);
                  const transcriptionText = (processingResult && typeof processingResult === 'object' && processingResult !== null && 'fullVideoTranscription' in processingResult ? String(processingResult.fullVideoTranscription) : 
                                           processingResult && typeof processingResult === 'object' && processingResult !== null && 'transcription' in processingResult ? String(processingResult.transcription) : '') || '';
          ...
          showToast(`Video processed successfully! Generated ${(processingResult as any).clipsGenerated || (processingResult as any).clips_generated} viral clips.`, 'success');
        ...
        if (!preflightOk) {
          ...
          // Use focused backend if available, otherwise fallback to legacy
          const processVideoFunction = processVideoFocused;
          
          const processingResult = await processVideoFunction({
            projectName: title.trim(),
            description: description.trim() || '',
            sourceType: 'url',
            sourceUrl: url.trim(),
          ...
          if (processingResult) {
            console.log('✅ [UploadPage] Backend processing completed successfully');
            console.log(`📊 [UploadPage] Generated ${(processingResult as any).clipsGenerated || (processingResult as any).clips_generated} clips`);
          ...
          showToast(`YouTube video processed successfully! Generated ${(processingResult as any).clipsGenerated || (processingResult as any).clips_generated} viral clips.`, 'success');
      ...
      // Navigate to project details
        ...
        if (processingResult) {
          ...
          showToast(`Text processed successfully! Generated ${(processingResult as any).clipsGenerated || (processingResult as any).clips_generated} viral clips.`, 'success');
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-3">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="developer-mode" className="flex items-center space-x-2 text-sm font-medium text-gray-300">
                    <Shield className="h-4 w-4 text-orange-400" />
                    <span>Developer Mode</span>
                    {developerMode && <Crown className="h-4 w-4 text-yellow-400" />}
                  </label>
                </div>
                                 {developerMode && (
                   <div className="px-3 py-1 bg-orange-500/20 border border-orange-500/30 rounded-lg">
                     <span className="text-orange-300 text-xs font-medium">🚀 UNLIMITED PROCESSING</span>
                   </div>
                 )}
               </div>
             </div>
             ...
             {developerMode && (
               <div className="mb-4 p-4 bg-orange-900/50 border border-orange-500/50 rounded-lg">
                 <div className="flex items-center space-x-2 mb-2">
                   ...
                   <span className="text-orange-300 font-medium">Developer Mode Active</span>
                 </div>
                 <ul className="text-orange-200 text-sm space-y-1 ml-7">
                   <li>• ✅ <strong>Unlimited video processing</strong> - No monthly limits</li>
                   <li>• ✅ <strong>Bypass subscription checks</strong> - Process any amount of content</li>
                   <li>• ✅ <strong>Full feature access</strong> - All AI features enabled</li>
                   <li>• ⚠️ <strong>Development only</strong> - Use for testing and development</li>
                                   </ul>
                </div>
              )}
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {uploadMethod === 'file' && (
              <div className="bg-green-900/20 p-6 rounded-xl border border-green-500/30">
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <span className="text-green-400 font-medium text-lg">🎬 Direct Video Upload</span>
                </div>
                <p className="text-green-300 text-sm leading-relaxed">
                  Your video will be uploaded directly to the backend as a complete file for immediate processing. 
                  Simple, fast, and reliable - no chunking required.
                </p>
                <div className="mt-3 text-xs text-green-400">
                  💡 <strong>How it works:</strong> Complete video → Direct upload → Processing starts immediately
                </div>
              </div>
            )}
                    ...
                    <div className="col-span-2">
                        ...
                        <div className="mt-4 space-y-4">
                                ...
                                <div className="flex items-center justify-between p-4 bg-gradient-to-r from-amber-900/20 to-orange-900/20 border border-amber-500/30 rounded-lg">
                                    <div className="flex items-center gap-3">
                                        <input
                                            ...
                                            onChange={(e) => {
                                                if (e.target.checked) {
                                                    // 🛠️ Allow watermark removal in developer mode
                                                    if (developerMode) {
                                                        setProcessingOptions(prev => ({
                                                            ...prev,
                                                            enableWatermark: false
                                                        }));
                                                        showToast('🛠️ Developer Mode: Watermark removed', 'success');
                                                    } else {
                                ...
                                <p className="text-xs text-amber-200 mt-2 ml-7">
                                    {developerMode 
                                        ? '🛠️ Developer Mode: Watermark removal enabled for testing' 
            ...
            <div className="flex items-center justify-between">
              ...
              {focusedBackendProcessing && focusedProgressMessage && (
                <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded-lg">
                  <div className="flex items-center space-x-3">
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-400"></div>
                    <div className="flex-1">
                      <p className="text-blue-300 font-medium">{focusedProgressMessage}</p>
                      {focusedEstimatedTime && focusedEstimatedTime > 0 && (
                        <p className="text-blue-400 text-sm mt-1">
                          Estimated time remaining: {Math.ceil(focusedEstimatedTime / 60)} minutes
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <button
                type="submit"
                disabled={processing}
                className="px-8 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700"
              >
