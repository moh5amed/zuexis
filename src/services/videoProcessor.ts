...
class EnhancedVideoProcessor {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      console.log('🎬 [VideoProcessor] Initializing FFmpeg for enhanced processing...');
      ...
      console.error('❌ [VideoProcessor] Failed to initialize FFmpeg:', error);
      
      // Provide fallback options
      if (error instanceof Error && error.message.includes('timeout')) {
        console.warn('⚠️ [VideoProcessor] FFmpeg timeout - using fallback processing');
  ...
  private async initializeWhisper(): Promise<void> {
    try {
      console.log('🎤 [VideoProcessor] Initializing Whisper for audio analysis...');
      ...
      try {
        ...
        console.log('✅ [VideoProcessor] Whisper Transcriber loaded successfully');
      } catch (whisperError) {
        console.warn('⚠️ [VideoProcessor] Whisper not available, using fallback:', whisperError);
      ...
      if (!this.isWhisperLoaded && 'webkitSpeechRecognition' in window) {
        console.log('🎤 [VideoProcessor] Using Web Speech API as fallback');
      }

    } catch (error) {
      console.warn('⚠️ [VideoProcessor] Whisper initialization failed, continuing without audio transcription:', error);
      this.isWhisperLoaded = false;
    }
  }
  ...
  async analyzeVideoWithSceneDetection(videoFile: ArrayBuffer | LocalFile): Promise<SceneDetectionResult> {
    ...
    console.log('🎬 [VideoProcessor] Starting enhanced video analysis with scene detection...');
    ...
    try {
      ...
      console.log('📁 [VideoProcessor] Video file written to FFmpeg');

      // Get video duration and basic info
      const duration = await this.getVideoDuration();
      console.log(`⏱️ [VideoProcessor] Video duration: ${duration}s`);

      // Scene detection using FFmpeg
      const scenes = await this.detectScenes(duration);
      console.log(`🎬 [VideoProcessor] Detected ${scenes.length} potential scenes`);
      ...
      console.log(`🎵 [VideoProcessor] Comprehensive audio analysis complete`);
      ...
      console.log(`📊 [VideoProcessor] Created ${segments.length} enhanced video segments`);
  ...
  async analyzeAudioComprehensive(videoFile: ArrayBuffer, duration: number): Promise<SceneDetectionResult['audioAnalysis']> {
    try {
      console.log('🎵 [VideoProcessor] Starting comprehensive audio analysis with Whisper...');
      
      // 1. Basic audio energy analysis (FFmpeg)
      const basicAudio = await this.analyzeAudioEnergy();
      console.log('🔊 [VideoProcessor] Basic audio energy analysis complete');

      // 2. Extract audio for Whisper processing
      const audioBuffer = await this.extractAudioForWhisper(videoFile);
      console.log('🎤 [VideoProcessor] Audio extracted for Whisper processing');
      ...
      if (this.isWhisperLoaded && audioBuffer) {
        try {
          ...
          console.log('🎤 [VideoProcessor] Whisper analysis complete');
        } catch (whisperError) {
          console.warn('⚠️ [VideoProcessor] Whisper analysis failed, using defaults:', whisperError);
        }
      } else {
        console.log('⚠️ [VideoProcessor] Whisper not available, using basic audio analysis only');
      ...
      console.error('❌ [VideoProcessor] Comprehensive audio analysis failed:', error);
  ...
  private async extractAudioForWhisper(videoFile: ArrayBuffer): Promise<ArrayBuffer | null> {
    try {
      console.log('🎤 [VideoProcessor] Extracting audio for Whisper processing...');
      ...
      console.log('✅ [VideoProcessor] Audio extracted successfully for Whisper');
      return audioData;

    } catch (error) {
      console.error('❌ [VideoProcessor] Audio extraction failed:', error);
      return null;
    }
  }
  ...
  private async analyzeAudioWithWhisper(audioBuffer: ArrayBuffer, duration: number): Promise<{
    ...
    try {
      console.log('🎤 [VideoProcessor] Running Whisper analysis...');
      ...
      console.log('📝 [VideoProcessor] Whisper model loaded successfully');
      ...
      console.error('❌ [VideoProcessor] Whisper analysis failed:', error);
      throw error;
    }
  }
  ...
  private analyzeTranscriptionInsights(transcription: any, duration: number): {
    ...
    try {
      ...
      console.warn('⚠️ [VideoProcessor] Transcription analysis failed, using defaults:', error);
      return {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      if (this.isWhisperLoaded && duration > 10) { // Only analyze segments longer than 10s
        try {
          ...
          console.warn(`⚠️ [VideoProcessor] Could not get Whisper data for segment ${i}:`, error);
        }
      }
  ...
  private async getSegmentWhisperData(videoFile: ArrayBuffer, startTime: number, endTime: number): Promise<VideoSegment['whisperData']> {
    try {
      ...
      console.warn(`⚠️ [VideoProcessor] Segment Whisper analysis failed:`, error);
      return undefined;
    }
  }
  ...
  private async extractAudioSegment(videoFile: ArrayBuffer, startTime: number, endTime: number): Promise<Blob | null> {
    try {
      ...
      console.error('❌ [VideoProcessor] Audio segment extraction failed:', error);
      return null;
    }
  }
  ...
  private async getVideoDuration(): Promise<number> {
    try {
      ...
      console.warn('⚠️ [VideoProcessor] Could not determine exact duration, using estimate');
  ...
  private async detectScenes(duration: number): Promise<{ start: number; end: number }[]> {
    try {
      console.log('🎬 [VideoProcessor] Running scene detection algorithm...');
      ...
      if (sceneMatches && sceneMatches.length > 0) {
        ...
        console.log(`🎬 [VideoProcessor] Scene detection found ${scenes.length} valid scenes`);
        return scenes;
      }

      // Fallback: create segments based on duration
      console.log('⚠️ [VideoProcessor] Scene detection failed, using fallback segmentation');
      return this.createFallbackSegments(duration);

    } catch (error) {
      console.warn('⚠️ [VideoProcessor] Scene detection failed, using fallback:', error);
      return this.createFallbackSegments(duration);
    }
  }
  ...
  private async analyzeAudioEnergy(): Promise<{ averageEnergy: number; peakEnergy: number; energyVariance: number }> {
    try {
      console.log('🎵 [VideoProcessor] Analyzing audio energy levels...');
      ...
      console.warn('⚠️ [VideoProcessor] Audio analysis failed, using defaults:', error);
  ...
  async generateThumbnail(videoFile: ArrayBuffer, time: number): Promise<ArrayBuffer> {
    ...
    try {
      console.log(`🖼️ [VideoProcessor] Generating thumbnail at ${time}s`);
      ...
      // Provide fallback options
      if (error instanceof Error && error.message.includes('timeout')) {
        console.warn('⚠️ [VideoProcessor] Thumbnail generation timeout - using fallback method');
  ...
  async createVideoClip(
    ...
    try {
      console.log(`✂️ [VideoProcessor] Creating clip from ${startTime}s to ${endTime}s`);
      ...
      console.error('❌ [VideoProcessor] Clip creation failed:', error);
      
      // Provide fallback options
      if (error instanceof Error && error.message.includes('timeout')) {
        console.warn('⚠️ [VideoProcessor] Clip creation timeout - using fallback method');
  ...
  async cleanup(): Promise<void> {
    if (this.ffmpeg) {
      try {
        ...
        console.warn('⚠️ [VideoProcessor] Cleanup warning:', error);
      }
    }
    ...
    if (this.whisperModel) {
      try {
        ...
        this.isWhisperLoaded = false;
        console.log('🧹 [VideoProcessor] Whisper cleanup completed');
      } catch (error) {
        console.warn('⚠️ [VideoProcessor] Whisper cleanup warning:', error);
      }
    }
  }
}
