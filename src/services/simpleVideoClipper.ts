...
export class SimpleVideoClipper {
  ...
  constructor() {
    console.log('🎬 [SimpleVideoClipper] Initializing lightweight video clipper...');
  }

  async initialize(): Promise<void> {
    this.isInitialized = true;
    console.log('✅ [SimpleVideoClipper] Ready for clipping');
  }
  ...
  async createVideoClip(
    ...
    console.log(`🎬 [SimpleVideoClipper] Creating clip from ${startTime}s to ${endTime}s`);
    console.log(`🎯 [SimpleVideoClipper] Options: Quality=${options.quality}, Audio=${options.audio}, Format=${options.format}`);
    ...
    try {
      ...
      console.error('❌ [SimpleVideoClipper] Clip creation failed:', error);
  ...
  private async startRecording(
    ...
    try {
      ...
      if (options.audio) {
        try {
          console.log('🔊 [SimpleVideoClipper] Attempting simplified audio capture...');
          ...
          if ((video as any).captureStream) {
            ...
            if (audioTracks.length > 0) {
              ...
              console.log(`✅ [SimpleVideoClipper] Audio captured: ${audioTracks.length} tracks`);
            } else {
              console.log('⚠️ [SimpleVideoClipper] No audio tracks found in video stream');
            }
          } else {
            console.log('⚠️ [SimpleVideoClipper] captureStream not supported, continuing without audio');
          }
          
        } catch (error) {
          console.warn('⚠️ [SimpleVideoClipper] Audio capture failed:', error);
          console.log('🔊 [SimpleVideoClipper] Continuing without audio');
        }
      }
      ...
      console.log(`🎬 [SimpleVideoClipper] MediaRecorder created with ${combinedStream.getTracks().length} tracks`);
      console.log(`🎬 [SimpleVideoClipper] Video tracks: ${combinedStream.getVideoTracks().length}, Audio tracks: ${combinedStream.getAudioTracks().length}`);
  ...
  async analyzeVideoWithSceneDetection(videoFile: ArrayBuffer): Promise<{
    ...
    console.log('🎬 [SimpleVideoClipper] Starting simple video analysis...');
    ...
    try {
      ...
      console.log(`⏱️ [SimpleVideoClipper] Video duration: ${duration}s`);

      // Create simple segments based on duration
      const segments = this.createSimpleSegments(duration);
      console.log(`🎬 [SimpleVideoClipper] Created ${segments.length} segments`);
      ...
      console.error('❌ [SimpleVideoClipper] Video analysis failed:', error);
  ...
  async generateThumbnail(videoFile: ArrayBuffer, time: number = 0): Promise<string> {
    try {
      ...
      console.error('❌ [SimpleVideoClipper] Thumbnail generation failed:', error);
  ...
  async cleanup(): Promise<void> {
    ...
    console.log('🧹 [SimpleVideoClipper] Cleanup completed');
  }
}
