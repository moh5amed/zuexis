...
class CloudStorageService {
  ...
  async getConnectedProviders(): Promise<CloudStorageProvider[]> {
    ...
    try {
      ...
      if (!connectionTest) {
        console.log('⚠️ [CloudStorage] Connection test failed, but continuing with request...');
      }
    } catch (error) {
      console.log('⚠️ [CloudStorage] Connection test error, but continuing with request...', error);
    }
  ...
  private async exchangeCodeForTokens(authCode: string): Promise<{ access_token: string; refresh_token: string; expires_in: number }> {
    try {
      ...
      const response = await fetch(`https://zuexisbacckend.onrender.com/api/frontend/google-oauth`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  private async getGoogleUserInfo(accessToken: string): Promise<{ email: string; name: string }> {
    try {
      ...
      console.log('🔍 [CloudStorage] Access token preview:', accessToken.substring(0, 20) + '...');
